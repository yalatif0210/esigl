...
public class CalculationService {
  ...
  public void fillFieldsForInitiatedRequisition(Rnr requisition, ProgramRnrTemplate rnrTemplate, RegimenTemplate regimenTemplate) {
    List<ProcessingPeriod> fivePreviousPeriods = processingScheduleService.getNPreviousPeriodsInDescOrder(requisition.getPeriod(), requisition.getProgram().getAmcMonths().intValue());
    ...
    int secondPeriodIndex = requisition.getProgram().getAmcMonths().intValue() / 2;
  ...
  private void fillPreviousNCsInLineItems(Rnr requisition, Integer numberOfMonths, Date trackingDate) {
    if (numberOfMonths >= requisition.getProgram().getAmcMonths() && !(requisition.isEmergency() || requisition.isForVirtualFacility())) {
      return;
    }

    for (RnrLineItem lineItem : requisition.getFullSupplyLineItems()) {
      List<RnrLineItem> previousLineItems = requisitionRepository.getAuthorizedRegularUnSkippedLineItems(lineItem.getProductCode(),
          requisition, getNumberOfPreviousNCToTrack(numberOfMonths, requisition.getProgram().getAmcMonths().intValue()), trackingDate);
  ...
  private Integer getNumberOfPreviousNCToTrack(Integer m, Integer monthsInAMC) {
    return (m == 1) ? monthsInAMC - 1 : monthsInAMC / 2;
  }
