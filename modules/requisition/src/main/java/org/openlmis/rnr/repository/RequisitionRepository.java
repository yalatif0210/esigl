...
public class RequisitionRepository {
  ...
  private void updateFullSupplyLineItems(Rnr requisition) {
    for (RnrLineItem lineItem : requisition.getFullSupplyLineItems()) {
      ...
      insertLossesAndAdjustmentsForLineItem(lineItem, requisition.getFacility());
    }
  }
  ...
  private void insertLossesAndAdjustmentsForLineItem(RnrLineItem lineItem, Facility facility) {
    for (LossesAndAdjustments lossAndAdjustment : lineItem.getLossesAndAdjustments()) {

      if (lossAndAdjustment.getFacility() == null) {

        FacilityLight facilityLight = new FacilityLight();
        facilityLight.setFacilityType(facility.getFacilityType().getName());
        facilityLight.setId(facility.getId());
        facilityLight.setDistrict(facility.getGeographicZone().getName());
        facilityLight.setCode(facility.getCode());
        lossAndAdjustment.setFacility(facilityLight);

      }

      lossesAndAdjustmentsMapper.deleteBy(lineItem.getId(), lossAndAdjustment.getType().getName(), lossAndAdjustment.getFacility().getId());
  ...
  public List<Rnr> getApprovedRequisitionsForCriteriaAndPageNumber(String searchType, String searchVal, LocalDate startDate, LocalDate endDate, Integer pageNumber,
                                                                   Integer pageSize, Long userId, String rightName, String sortBy,
                                                                   String sortDirection) {
    return requisitionMapper.getApprovedRequisitionsForCriteriaAndPageNumber(searchType, searchVal, startDate, endDate, pageNumber, pageSize,
            userId, rightName, sortBy, sortDirection);
  }

  public Integer getCountOfApprovedRequisitionsForCriteria(String searchType, String searchVal, LocalDate startDate, LocalDate endDate, Long userId, String rightName) {
    return requisitionMapper.getCountOfApprovedRequisitionsForCriteria(searchType, searchVal, startDate, endDate, userId, rightName);
  }
