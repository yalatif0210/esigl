...
import org.openlmis.core.domain.FacilityLight;
...
public interface LossesAndAdjustmentsMapper {
  ...
  @Insert("INSERT INTO requisition_line_item_losses_adjustments(requisitionLineItemId, type, quantity, modifiedBy, facilityId, reason) " +
      "VALUES(#{rnrLineItem.id}, #{lossesAndAdjustments.type.name}, #{lossesAndAdjustments.quantity}, #{rnrLineItem.modifiedBy}, #{lossesAndAdjustments.facility.id}, #{lossesAndAdjustments.reason})")
  ...
  @Results(value = {
      ...
      @Result(property = "facility", column = "facilityId", javaType = Long.class, one = @One(select = "getLightFacilityById"))
  })
  List<LossesAndAdjustments> getByRnrLineItem(Long rnrLineItemId);

  @Select("select f.id, f.name, t.name as facilityType, d.name district from facilities f " +
      " join facility_types t on t.id = f.typeid " +
      " join geographic_zones d on f.geographiczoneid = d.id" +
      " WHERE f.id = #{id}" )
  FacilityLight getLightFacilityById(@Param("id") Long id);
  ...
  @Delete("DELETE FROM requisition_line_item_losses_adjustments WHERE requisitionLineItemId = #{rnrLineItemId} " +
      " AND type=#{type} AND facilityId = #{facilityId}")
  void deleteBy(@Param("rnrLineItemId") Long rnrLineItemId,
