  ...
  public class ApprovedRequisitionSearch {
    ...
    private static void appendQueryClausesBySearchType(StringBuilder sql, Map<String, Object> params) {
      ...
      if (!searchVal.isEmpty()) {
        if (searchType.isEmpty() || searchType.equalsIgnoreCase(RequisitionService.SEARCH_ALL)) {
          sql.append(" (LOWER(P.name) LIKE '%" + searchVal + "%' OR LOWER(F.name) LIKE '%" +
          ...
          sql.append("LOWER(F.code) LIKE '%" + searchVal + "%' AND ");
        } else if (searchType.equalsIgnoreCase(RequisitionService.SEARCH_FACILITY_NAME)) {
          sql.append("LOWER(F.name) LIKE '%" + searchVal + "%' AND ");
        } else if (searchType.equalsIgnoreCase(RequisitionService.SEARCH_PROGRAM_NAME)) {
          sql.append("LOWER(P.name) LIKE '%" + searchVal + "%' AND ");
        } else if (searchType.equalsIgnoreCase(RequisitionService.SEARCH_SUPPLYING_DEPOT_NAME)) {
          sql.append("LOWER(SF.name) LIKE '%" + searchVal + "%' AND ");
        }
      }
      if(startDate != null){
        sql.append("PP.startDate >= '" + startDate.atStartOfDay() + "' AND ");
      }
      if(endDate != null){
        sql.append("PP.endDate <= '" + endDate.atTime(LocalTime.MAX) + "' AND ");
      }
