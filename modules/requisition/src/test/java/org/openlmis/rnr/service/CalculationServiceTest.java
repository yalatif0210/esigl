...
public class CalculationServiceTest {
  ...
  public void shouldGetPreviousOneNormalizedConsumptionFor3MonthsInCurrentPeriodIfPreviousPeriodNotExists() throws Exception {
    ...
    verify(processingScheduleService).getNPreviousPeriodsInDescOrder(requisition.getPeriod(), requisition.getProgram().getAmcMonths().intValue());
  ...
  public void shouldGetPreviousOneNormalizedConsumptionFor2MonthsInCurrentPeriodIfPreviousPeriodNotExists() throws Exception {
    ...
    when(processingScheduleService.getNPreviousPeriodsInDescOrder(requisition.getPeriod(), requisition.getProgram().getAmcMonths().intValue())).thenReturn(emptyPeriodList);
    RnrLineItem rnrLineItem = new RnrLineItem();
    rnrLineItem.setNormalizedConsumption(4);
    List<RnrLineItem> rnrLineItems = asList(rnrLineItem);
    when(requisitionRepository.getAuthorizedRegularUnSkippedLineItems(productCode, requisition, 1, requisition.getPeriod().getStartDate())).thenReturn(rnrLineItems);
  ...
  public void shouldGetPreviousOneNormalizedConsumptionFor3MonthsInPreviousPeriod() throws Exception {
    ...
    when(processingScheduleService.getNPreviousPeriodsInDescOrder(requisition.getPeriod(), requisition.getProgram().getAmcMonths().intValue())).thenReturn(asList(previousPeriod));
    RnrLineItem rnrLineItem = new RnrLineItem();
    rnrLineItem.setNormalizedConsumption(4);
    List<RnrLineItem> rnrLineItems = asList(rnrLineItem);
  ...
  public void shouldGetPreviousOneNormalizedConsumptionFor2MonthsInPreviousPeriod() throws Exception {
    ...
    when(processingScheduleService.getNPreviousPeriodsInDescOrder(requisition.getPeriod(), requisition.getProgram().getAmcMonths().intValue())).thenReturn(asList(previousPeriod, secondLastPeriod));
    RnrLineItem rnrLineItem = new RnrLineItem();
    rnrLineItem.setNormalizedConsumption(4);
    List<RnrLineItem> rnrLineItems = asList(rnrLineItem);
  ...
  public void shouldGetPreviousOneNormalizedConsumptionFor2MonthsInPreviousPeriodAndShouldTrackFromLast2Periods() throws Exception {
    ...
    when(processingScheduleService.findM(requisition.getPeriod())).thenReturn(2);
    when(processingScheduleService.getNPreviousPeriodsInDescOrder(requisition.getPeriod(), requisition.getProgram().getAmcMonths().intValue())).thenReturn(asList(previousPeriod, secondLastPeriod, thirdLastPeriod));
    RnrLineItem rnrLineItem = new RnrLineItem();
    rnrLineItem.setNormalizedConsumption(4);
    List<RnrLineItem> rnrLineItems = asList(rnrLineItem);
    when(requisitionRepository.getAuthorizedRegularUnSkippedLineItems(productCode, requisition, 1, trackingStartDate)).thenReturn(rnrLineItems);
  ...
  public void shouldGetPreviousTwoNormalizedConsumptionsFor1MonthInPreviousPeriodAndShouldTrackFromLast5Periods() throws Exception {
    ...
    when(processingScheduleService.findM(requisition.getPeriod())).thenReturn(1);
    when(processingScheduleService.getNPreviousPeriodsInDescOrder(requisition.getPeriod(), requisition.getProgram().getAmcMonths().intValue())).thenReturn(asList(previousPeriod, secondLastPeriod, thirdLastPeriod, fourthLastPeriod, fifthLastPeriod));
    RnrLineItem rnrLineItem = new RnrLineItem();
    rnrLineItem.setNormalizedConsumption(4);
    RnrLineItem rnrLineItem2 = new RnrLineItem();
  ...
  public void shouldNotTrackPreviousRequisitionsIfRegularRnrAndMIs3() throws Exception {
    ...
    when(processingScheduleService.getNPreviousPeriodsInDescOrder(spyRnr.getPeriod(), rnr.getProgram().getAmcMonths().intValue())).thenReturn(asList(previousPeriod, secondLastPeriod));
    ...
    verify(processingScheduleService).getNPreviousPeriodsInDescOrder(spyRnr.getPeriod(), rnr.getProgram().getAmcMonths().intValue());
