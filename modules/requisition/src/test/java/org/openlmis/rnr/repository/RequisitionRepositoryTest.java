...
public class RequisitionRepositoryTest {
 ...
 // @Test
  ...
  public void shouldGetApprovedRequisitionsForCriteriaAndPageNumber() throws Exception {
    ...
    Integer pageSize = 2;
    when(requisitionMapper.getApprovedRequisitionsForCriteriaAndPageNumber(searchType, searchVal, null, null,  pageNumber, pageSize,
            1l, CONVERT_TO_ORDER, sortBy, sortDirection)).thenReturn(expected);

    List<Rnr> rnrList = requisitionRepository.getApprovedRequisitionsForCriteriaAndPageNumber(searchType, searchVal, null, null,
            pageNumber, pageSize, 1l, CONVERT_TO_ORDER, sortBy, sortDirection);

    assertThat(rnrList, is(expected));
    verify(requisitionMapper).getApprovedRequisitionsForCriteriaAndPageNumber(searchType, searchVal, null, null, pageNumber,
  ...
  public void shouldGetCountOfApprovedRequisitionsForCriteriaAndPageNumber() throws Exception {
    ...
    String searchType = SEARCH_ALL;
    String searchVal = "test";
    when(requisitionMapper.getCountOfApprovedRequisitionsForCriteria(searchType, searchVal, null, null, 1l, CONVERT_TO_ORDER)).thenReturn(requisitionCount);

    Integer result = requisitionRepository.getCountOfApprovedRequisitionsForCriteria(searchType, searchVal, null, null, 1l, CONVERT_TO_ORDER);

    assertThat(result, is(requisitionCount));
    verify(requisitionMapper).getCountOfApprovedRequisitionsForCriteria(searchType, searchVal, null, null, 1l, CONVERT_TO_ORDER);
  }
