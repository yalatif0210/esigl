...
function ApproveRnrListController($scope, requisitionList, $location, messageService) {
  ...
  $scope.filterRequisitions = function () {
    ...
    $scope.filteredRequisitions = $.grep($scope.requisitions, function (rnr) {
      var isFiltered=(searchField) ? contains(rnr[searchField], query) : matchesAnyField(query, rnr);
      if(startDate){
        isFiltered =isFiltered && rnr.periodStartDate >= (new Date(startDate)).getTime();
      }
      if(endDate){
        isFiltered =isFiltered && rnr.periodEndDate <= (new Date(endDate)).setUTCHours(23,59,59);
      }
      return isFiltered;
    });
