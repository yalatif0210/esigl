...
function ConvertToOrderListController($scope, Orders, RequisitionForConvertToOrder,SupplyingDepots, $dialog, messageService, $routeParams, $location) {
  ...
  var refreshGrid = function () {
    ...
    RequisitionForConvertToOrder.get({page: $scope.currentPage, searchType: $scope.selectedSearchOption.value,
          searchVal: $scope.query, sortBy: $scope.sortOptions.fields[0], sortDirection: $scope.sortOptions.directions[0],startDate:$scope.startDate,endDate:$scope.endDate},
        function (data) {
  ...
  $scope.updateSearchParams = function () {
    ...
    $location.search({page: 1, searchType: $scope.selectedSearchOption.value, searchVal: $scope.query || '',startDate:$scope.startDate,endDate:$scope.endDate});
  };
