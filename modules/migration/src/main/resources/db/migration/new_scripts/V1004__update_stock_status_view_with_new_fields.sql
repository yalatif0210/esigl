-- Update stock status view to include new fields for API enhancement
-- Add facilitycode as esigl_code, region_name as region, interface_dataset.datasetname as id_DHIS2
...
SELECT r.emergency,
       f.id facilityid,
       f.name facilityname,
       f.code as esigl_code,
       f.geographiczoneid,
       dis.district_name AS districtname,
       gz.name AS region_name,
       p.code programcode,
       p.name programname,
...
FROM requisition_line_items d
         ...
         LEFT JOIN interface_dataset id ON f.code = id.datasetid
