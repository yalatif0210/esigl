...
public interface ELMISInterfaceMapper {
   ...
   @Select("select set.datasetId from interface_dataset set\n" +
        "    JOIN interface_apps apps ON set.interfaceId = apps.id where lower(apps.name) = lower(#{sageConfig}) and lower(set.datasetName) = lower(#{code}) limit 1")
    String getDataSageByProgramCode(@Param("code") String code, @Param("sageConfig") String sageConfig);

    @Select("select set.datasetname from interface_dataset set\n" +
        "    JOIN interface_apps apps ON set.interfaceId = apps.id where apps.id = 4 and set.datasetId = #{facilityCode} limit 1")
    String getDHIS2IdByFacilityCode(@Param("facilityCode") String facilityCode);
}
