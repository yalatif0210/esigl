...
public interface FacilityMapper {
  ...
  @Select("select f.id, f.name, t.name as facilityType, d.name district, f.code from facilities f " +
      " join facility_types t on t.id = f.typeid " +
      " join geographic_zones d on f.geographiczoneid = d.id")
  List<FacilityLight> getLightFacilityList();

  @Select("select f.id, f.name, t.name as facilityType, d.name district from facilities f " +
      " join facility_types t on t.id = f.typeid " +
      " join geographic_zones d on f.geographiczoneid = d.id " +
      " WHERE f.id in (select facilityId from programs_supported where programId = #{programId})")
  List<FacilityLight> getLightFacilityListBySupportedProgram(@Param("programId")Long programId);
